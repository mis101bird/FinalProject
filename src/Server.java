...
public class Server {
    ...
    public static void main(String[] args) {
       ...
       post("/adminaccount/:uid", (request, response) -> {
       System.out.println("go adminaccount successfully");
    ...
    System.out.println("adminaccount: Open database successfully");                                                             
    try{
    Statement stmt = null;    
    Class.forName("org.sqlite.JDBC");                                               
    stmt = c.createStatement(); 
    String sql1="INSERT INTO ADMIN (UID) " +
            "VALUES ("+uid+");";                                                                     
    ...
    stmt.executeUpdate(sql1);
    sql1="DELETE from ADMIN where UID='"+uid+"';";                                                           
    stmt.executeUpdate(sql1);
    stmt.close();
     c.close();
   ...
   String word="";
    ...
    System.out.println("Admin: Open database successfully");
    try{
    Statement stmt = null;
    ...
    book = stmt2.executeQuery(sql2);
    String sql3 = "SELECT * FROM ADMIN;";
    admin = stmt3.executeQuery(sql3);
    word=Index.admin(user , book , admin);
     book.close(); 
     user.close(); 
     admin.close(); 
     stmt.close();
   ...
   System.out.println("In Admin database wrong: "+e.getMessage());
   }
        return word;
   ...
   String word="";
    ...
    String sql3 = "SELECT * FROM ADMIN;";
    ad = stmt1.executeQuery(sql3);
    word=Index.afterLogin(bo,re);
    while(ad.next()){
    String a= ad.getString("UID");
    if((""+uid).equals(a)){
     word+="<a href='admin' class='btn btn-primary btn-primary'>Admin</a>";
     break;
    }
    }
    word+="</body></html>";
